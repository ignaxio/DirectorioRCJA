...
class VistaUsuarioRCJA extends VistaAbstractaRCJA {
        ...
        public function printVista($vista) {
                if ($vista == "resumen")
                        ...
                        $this->printLista();
                else if ($vista == "lista_usuarios_dos_sistemas")
                        $this->printListaUsuariosDosSistemas();
                else if ($vista == "mini")
                        $this->printMini();
...
// print_r($param);
                ...
                if ($vista == "lista_usuarios_dos_sistemas") {
                        echo '<form class="DirectorioRCJA_listadoDoble" action="demo_form.asp" method="post">';
                        if ($cuenta < 1) {
                                DirectorioRCJA::tabla_antes('Se han encontrado ' . $cuenta . ' USUARIO(S)', '0');
                                echo "<table>";
                                echo "<tr><td class='DirectorioRCJA_fontBlack'><img src='/MODULOS/DirectorioRCJA/images/important.png' width=48px>NO SE HAN ENCONTRADO RESULTADOS</td></tr>";
                        } else {
                                $patron = isset($param['patron']) ? $this->getPatron($param) : '';
                                ?>
                                <script language='JavaScript'>
                                        SuperAjax.prototype.DirectorioRCJA_ordena = function (campo, orden) {
                                                url = '/dispatcherGET.php?class=DirectorioRCJA&method=reentrada&subclass=FichaUsuarioRCJA&submethod=body&<? echo $patron; ?>&vista=lista_usuarios_dos_sistemas&ajax=true&campo=' + campo + '&orden=' + orden;
                                                new Ajax.Updater('layout_body', url, {evalScripts: 'yes'});
                                        };
                                </script>
                                <?
                                ...
                                echo "<table class='DirectorioRCJA_listatable' style='width: 100%;'>";
                                ?> 
                                <tr  class='DirectorioRCJA_table_head_listado_doble'>
                                        <td colspan="3" class='DirectorioRCJA_lineaDerecha'>Usuarios de TAU</td>
                                        <td class='DirectorioRCJA_lineaDerecha' colspan="3">Usuarios de RCJA</td>
                                        <td>Selector</td>
                                </tr>   
                                <?
                                echo "<tr class='DirectorioRCJA_table_head'>"
                                . "<td>DNI</td>"
                                . "<td>NOMBRE</td>"
                                . "<td class='DirectorioRCJA_lineaDerecha'>EMAIL</td>"
                                . "<td style='cursor: pointer' onClick='SpAJAX.DirectorioRCJA_ordena(1,{$ordenes[1]})'>EMPLEADO</td>"
                                . "<td style='cursor: pointer' onClick='SpAJAX.DirectorioRCJA_ordena(2,{$ordenes[2]})'>NOMBRE</td>"
                                . "<td class='DirectorioRCJA_lineaDerecha' style='cursor: pointer' onClick='SpAJAX.DirectorioRCJA_ordena(3,{$ordenes[3]})'>NIF</td>"
                                . "<td></td>"
                                . "</tr>";
                        }
                }
        ...
        public function printPostVista($vista) {
                ...
                if ($vista == "lista_usuarios_dos_sistemas") {
                        echo "</table>";
                        DirectorioRCJA::tabla_despues();
                        // Ahora hay que pintar los botones.
                        ?>
                        <div class="botones">
                                <input type="submit" name="actualizarUsuarioTAU" value="<< Actualizar usuario en TAU">
                                <input type="submit" name="mostrarDetalles" value="mostrarDetalles">
                                <input type="submit" name="actualizarUsuarioRCJA" value="Actualizar usuario en RCJA >>">
                        </div>
                        </form>
        ...
        public function printListaUsuariosDosSistemas() {
                echo "<tr onmouseover='this.style.background=\"#fffaaf\"' onMouseOut='this.style.background=\"\"'>";
                // Primero los usuarios de tau
                $usuarioTAU = new FichaUsuario();
                $Usuario = $this->getFicha()->arrayUsuariosDosSistemas[$this->getObjeto()->getEmpleado()];
                $usuarioTAU->body(Array('idusr' => $Usuario['idTAU'], 'vista' => 'lista_usuarios_dos_sistemas'));
                // Ahora los usuarios en RCJA
                ...
                echo "<td class='DirectorioRCJA_fontBlack DirectorioRCJA_lineaDerecha'><a class='DirectorioRCJA_fontBlack' href='/dispatcherGET.php?class=DirectorioRCJA&method=reentrada&subclass=FichaUsuarioRCJA&submethod=body&vista=ficha&idusr={$this->getObjeto()->getEmpleado()}'>{$this->getObjeto()->getNIF()}</a></td>";
                echo "<td class='DirectorioRCJA_fontBlack'><input type='checkbox' name='usuario' value='" . $Usuario['idTAU'] . "'><br></td>";
